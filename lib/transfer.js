...
var checkForExpiry = function (uploadUUID) {
  if (pieceMap[uploadUUID]) {
    if (moment(new Date()).diff(moment(pieceMap[uploadUUID].updateTime), 'milliseconds') >= pieceMap[uploadUUID].chunkExpiry) {
...
exports.upload = function (options, cb) {
  ...
  var uploadQueue = new Queue(function (job, queueCB) {
    ...
    if ((options.failAfter >= 0) && (numUploads >= options.failAfter)) return queueCB('Induced Failure from failAfter option');
  ...
  uploadQueue.on('drain', function () {
    uploadQueue.destroy();
    if (numUploads !== numPieces) return cb(`Upload of ${numPieces - numUploads} pieces failed`, uploadUUID);
    cb(null, uploadUUID);
  });
